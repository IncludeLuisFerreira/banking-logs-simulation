# Visualizador Gráfico de Concorrência — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rewrite the visual simulation page with event queue, speed slider, hybrid hub-and-spoke + account-to-account layout, and navigation between dashboard and visual page.

**Architecture:** SSE events → buffer queue → tick timer (100-2000ms, configured via slider) → state manager → 3-layer SVG renderer (hub lines, account cards, transaction arrows).

**Tech Stack:** Vanilla JS (EventSource, setInterval, Canvas-like DOM positioning), Express backend (unchanged), CSS animations.

---

### Task 1: Add navigation button on Dashboard

**Files:**
- Modify: `app/public/html/dashboard.html`
- Modify: `app/public/css/dashboard.css`

- [ ] **Step 1: Add button to dashboard HTML**

In `app/public/html/dashboard.html`, add the "Ver Simulação Visual" button inside `.sim-controls`, after the existing buttons:

```html
<div class="sim-controls">
  <button id="btnIniciarSim" class="btn btn--primary panel-btn">▶ Simulação NxN</button>
  <button id="btnPararSim" class="btn panel-btn btn--danger" disabled>■ Parar</button>
  <a href="simulacao-visual.html" class="btn btn--secondary panel-btn">📊 Ver Simulação Visual</a>
</div>
```

- [ ] **Step 2: Add CSS for the new button**

In `app/public/css/dashboard.css`, add style for `.btn--secondary`:
...
- [ ] **Step 3: Verify the dashboard loads and the link points to the correct page**

Run: `ls app/public/html/simulacao-visual.html app/public/html/dashboard.html`
Expected: Both files exist in the same directory.

---

### Task 2: Update visual page HTML + CSS

**Files:**
- Modify: `app/public/html/simulacao-visual.html`
- Modify: `app/public/css/simulacao-visual.css`

- [ ] **Step 1: Add speed slider and back button to HTML**

Replace the header and controls sections in `app/public/html/simulacao-visual.html`:
...
<div class="visual-controls">
  ...
  <input type="number" id="inputNumContas" class="form-input" value="8" min="5" max="15" style="width:80px">
  <button id="btnIniciar" class="btn btn--primary">▶ Iniciar</button>
  <button id="btnParar" class="btn btn--danger" disabled>■ Parar</button>
...
- [ ] **Step 2: Add CSS for slider, back button, and hub lines**

Add to `app/public/css/simulacao-visual.css`:
...
### Task 3: Rewrite simulacao-visual.js

**Files:**
- Rewrite: `app/public/js/simulacao-visual.js`

This is a complete rewrite. The file will have 4 internal modules:
...
let eventBuffer = [];
let tickTimer = null;
let tickInterval = 500; // default ms
...
function configurarVelocidade(ms) {
  tickInterval = ms;
  if (tickTimer) {
    iniciarTimer(); // restart with new interval
  }
}
...
let contasData = [];
let accountStates = new Map();   // contaId -> { hubLineState, borderState }
let activeArrows = new Map();    // "origem-destino" -> { arrowState, timeoutId }
...
function processarEvento(type, data) {
  ...
  if (type === 'lock:request') {
    ...
    const { contaId, origemId, destinoId } = data;
    if (contaId) setAccountState(contaId, type);
    if (origemId && destinoId) setArrowState(`${origemId}-${destinoId}`, type);
  }
    ...
    // Show success on arrow for 2s, then remove
    ...
    // Show success on hub lines for 2s, then return to idle
    if (origemId) setAccountState(origemId, 'idle');
    if (destinoId) setAccountState(destinoId, 'idle');
    // But briefly show green on hub lines
...
async function iniciarSimulacao() {
  ...
  try {
    ...
    // State initialization happens via SSE event simulacao-visual:iniciada
    // But we also need contasData for initial render
    contasData = data.contas || [];
...
Run: `node -c app/public/js/simulacao-visual.js`
Expected: `SyntaxError: ...` or silence (Node might not parse browser JS perfectly, but basic syntax check)
...
Run: `make start` or `node app/app.js`
Expected: Server starts on port 3000, "Banking Simulation API rodando em http://localhost:3000"

- [ ] **Step 2: Open visual page and start simulation**

Open `http://localhost:3000/html/simulacao-visual.html` (after login).
Set 8 accounts, click "▶ Iniciar".
Expected: Accounts appear in orbit, hub lines drawn from center to each account, simulation events processed at ~500ms intervals.

- [ ] **Step 3: Adjust speed slider**

Move the slider to 100ms and 2000ms.
Expected: Animation speed changes accordingly. At 2000ms events are very slow, at 100ms they're fast.

- [ ] **Step 4: Navigate back to dashboard**

Click "← Voltar ao Dashboard".
Expected: Navigates to `dashboard.html`, JWT preserved, user still authenticated.
